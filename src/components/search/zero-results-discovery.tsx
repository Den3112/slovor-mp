...
export function ZeroResultsDiscovery({ query, locale }: ZeroResultsDiscoveryProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <motion.div
        ...
        className="bg-primary/5 mb-8 flex h-24 w-24 items-center justify-center rounded-[2.5rem] text-primary shadow-inner"
      >
      ...
      <p className="text-muted-foreground mb-12 max-w-md font-medium">
        {t('noResultsDescription', { defaultValue: "We couldn't find matches for your search. Try different keywords or browse our trending categories." })}
      </p>
            ...
            {trendingCategories.map((cat) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-foreground text-sm font-bold">{cat.name}</span>
                </div>
        ...
        <div className="bg-primary shadow-primary/20 flex flex-col justify-center rounded-[2rem] p-8 text-left text-white shadow-2xl">
          ...
          <ul className="space-y-4 font-medium opacity-90">
            <li className="flex items-start gap-3">
              ...
              <span>{t('tip1', { defaultValue: 'Check for typos or spelling errors' })}</span>
            </li>
            <li className="flex items-start gap-3">
              <div className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-white/40" />
              <span>{t('tip2', { defaultValue: 'Try more general keywords' })}</span>
            </li>
            <li className="flex items-start gap-3">
              <div className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-white/40" />
              <span>{t('tip3', { defaultValue: 'Broaden your location search' })}</span>
            </li>
          </ul>
          <Button
            asChild
