...
describe('logger utility', () => {
  ...
  it('should log errors with correct prefix in development', () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith('[ERROR][err.context][client]:', 'test error')
  })

  it('should log warnings in development', () => {
    logWarn('warn.context', 'be careful')
    expect(warnSpy).toHaveBeenCalledWith('[WARN][warn.context][client]:', 'be careful')
  })

  it('should log info in development', () => {
    logInfo('info.context', 'some info')
    expect(infoSpy).toHaveBeenCalledWith('[INFO][info.context][client]:', 'some info')
  })
