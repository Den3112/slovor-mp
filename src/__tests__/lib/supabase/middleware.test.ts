...
describe('updateSession Middleware', () => {
  ...
  it('should handle lang prefix in paths', async () => {
    ...
    const response = await updateSession(request)
    expect(response.status).toBe(307)
    expect(response.headers.get('location')).toBe('http://localhost:3000/sk/login')
  })

  it('should protect /admin route from non-admin users', async () => {
    const request = mockRequest('/admin')
    vi.mocked(ssr.createServerClient).mockReturnValue(mockSupabase({ id: '1', email: 'user@test.com' }, 'user') as any)
  ...
  it('should allow /admin access for admins verified by DB role', async () => {
    const request = mockRequest('/admin')
    vi.mocked(ssr.createServerClient).mockReturnValue(mockSupabase({ id: '1' }, 'admin') as any)
  ...
  it('should allow /admin access via fallback adminEmails list', async () => {
    const request = mockRequest('/admin')
    vi.mocked(ssr.createServerClient).mockReturnValue(mockSupabase({ id: '1', email: 'admin@example.com' }, 'user') as any)
