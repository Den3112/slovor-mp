...
export function HowItWorks() {
  ...
  return (
    <section className="bg-background relative overflow-hidden py-24 md:py-32">
      ...
      <div className="bg-grid-slate-100 absolute inset-0 mask-[linear-gradient(to_bottom,white,transparent)] opacity-20 dark:opacity-10" />
        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-3 lg:gap-16">
          {steps.map((step, idx) => (
            ...
            >
              ...
              {idx < steps.length - 1 && (
                <div className="from-primary/40 absolute top-16 left-[60%] hidden h-[2px] w-[80%] bg-linear-to-r to-transparent md:block">
                  <motion.div
                    initial={{ scaleX: 0 }}
                    whileInView={{ scaleX: 1 }}
                    viewport={{ once: true }}
                    transition={{ duration: 1, delay: 0.5 + idx * 0.2 }}
                    className="bg-primary h-full w-full origin-left shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]"
                  />
                </div>
              )}
              ...
              <div className="relative mb-10">
                {/* Glow behind step */}
                <div className="bg-primary/20 absolute -inset-4 rounded-full opacity-0 blur-2xl transition-opacity duration-500 group-hover:opacity-40" />

                <div className="bg-card border-border group-hover:shadow-primary/20 relative flex h-20 w-20 items-center justify-center rounded-2xl border shadow-md transition-all duration-500 group-hover:scale-110 sm:h-24 sm:w-24">
                  <div
                    className={cn(
                  ...
                  {/* Step Number Badge */}
                  <div className="bg-card text-foreground border-border absolute -top-3 -right-3 flex h-10 w-10 items-center justify-center rounded-full border text-sm font-black shadow-md">
                    {idx + 1}
                  </div>
                </div>
              </div>
