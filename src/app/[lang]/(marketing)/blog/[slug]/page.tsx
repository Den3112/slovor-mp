...
export async function generateStaticParams() {
  ...
  const { data: posts } = await blogApi.listPosts(supabase, { limit: 10, offset: 0 })
  if (!posts) return []
...
export default async function BlogPostPage({ params }: BlogPostProps) {
  ...
  const { data: post, error } = await blogApi.getPostBySlug(slug, supabase)
          ...
          <aside className="space-y-6 lg:col-span-4">
            <div className="bg-card border-border rounded-xl border p-8 shadow-sm">
              ...
              <div className="flex gap-3">
                ...
                >
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(`https://slovor.sk/${lang}/blog/${slug}`)}&text=${encodeURIComponent(post.title)}`}
                    target="_blank"
                ...
                >
                  <a
                    href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://slovor.sk/${lang}/blog/${slug}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
