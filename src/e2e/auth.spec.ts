...
test.describe('Authentication Flow & UI', () => {
  ...
  test('Login page renders correctly with new UI fixes', async ({ page }) => {
    ...
    const headerSignIn = page
      .locator('header')
      .getByRole('link', { name: /Sign In|Prihlásiť/i })
    await expect(headerSignIn).not.toBeVisible()

    // Check Forgot Password link
    const forgotPassword = page.getByRole('button', {
      name: /Forgot password|Zabudnuté heslo|Забыли пароль/i,
    })
    await expect(forgotPassword).toBeVisible()

    // Check labels readability (should be visible)
    await expect(
      page.locator('label', { hasText: /Email|E-mail/i })
    ).toBeVisible()
    await expect(
      page.locator('label', { hasText: /Password|Heslo|Пароль/i })
    ).toBeVisible()
  })
