...
test.describe('Listing Lifecycle', () => {
  test('User can create a new listing', async ({ page }) => {
    ...
    await page
      .getByPlaceholder(/What are you selling?/i)
      .fill('Test Item Playwright')
    await page.getByPlaceholder(/^0$/).fill('100')
    await page
      .getByPlaceholder(/Tell buyers more about your item/i)
      .fill(
        'This is an automated test listing description. It should be long enough to pass validation.'
      )
    ...
    try {
      ...
      throw new Error(
        `Step 2 -> Step 3 transition failed. Errors: ${errors.join(', ')}`
      )
    }
    ...
    await page
      .getByRole('button', { name: /Select Images/i })
      .click({ force: true })
    ...
    const publishButton = page
      .locator('button')
      .filter({ hasText: /Publish|Uverejniť/i })
      .first()
    ...
    await expect(page).toHaveURL(/dashboard|success|listings/i, {
      timeout: 15000,
    })
