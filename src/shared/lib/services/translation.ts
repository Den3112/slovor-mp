...
export function isTranslationEnabled(): boolean {
  return (
    Boolean(process.env.NEXT_PUBLIC_DEEPL_API_KEY) ||
...
export async function translateText(
  ...
  const apiKey =
    process.env.NEXT_PUBLIC_DEEPL_API_KEY || process.env.DEEPL_API_KEY

  if (!apiKey) {
    console.warn(
      '[TranslationService] DEEPL_API_KEY not set, using original text'
    )
    return text
  }
...
export async function generateListingTranslations(
  ...
  if (!isTranslationEnabled()) {
    console.warn(
      '[TranslationService] Translation not enabled, using source text only'
    )
    return {
      title_en: title,
      title_sk: title,
