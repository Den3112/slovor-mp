...
describe('withRetry', () => {
  ...
  it('should fail after max retries if errors persist', async () => {
    const operation = vi
      .fn()
      .mockRejectedValue({ code: '503', message: 'Persistent Failure' })
  ...
  it('should not retry on non-retryable errors', async () => {
    const operation = vi
      .fn()
      .mockRejectedValue({ code: '400', message: 'Bad Request' })

    await expect(withRetry(operation, { initialDelay: 10 })).rejects.toThrow(
      'Bad Request'
    )
