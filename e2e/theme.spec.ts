...
test.describe('Theme Switching (Dark/Light Mode)', () => {
    ...
    for (const locale of locales) {
        test(`Dark Mode Toggle works on ${locale} Homepage`, async ({ page }) => {
            ...
            const isDarkAfterSecondClick = await html.evaluate((node) => node.classList.contains('dark'))

            // Assert they are different
            expect(isDark).not.toBe(isDarkAfterSecondClick)
        })

        test(`Dark Mode persists CSS variables on ${locale} Login Page`, async ({ page }) => {
    ...
    test('Theme persists across navigation', async ({ page }) => {
        ...
        const html = page.locator('html')
        const startDark = await html.evaluate((node) => node.classList.contains('dark'))
