...
async function globalSetup(config: FullConfig) {
    const storageStatePath = path.resolve(config.projects[0].use.storageState as string || 'e2e/.auth/user.json');
    ...
    try {
        ...
        if (serviceRoleKey && supabaseUrl) {
            console.log('Global Setup: Using Supabase Admin API to bypass rate limits...');
            ...
            if (error) {
                console.error('Global Setup: ❌ Failed to generate magic link:', error.message);
                throw error;
            }
            ...
            if (data?.properties?.action_link) {
                ...
                if (session) {
                    ...
                    console.warn('Global Setup: All verification strategies failed. Token might be invalid or expired immediately.');
                }

                // Promote user (unchanged logic)
                let userId = data.user?.id || session?.user?.id;
                try {
                    if (!userId) { // Fallback listing
                    ...
                    if (userId) {
                        ...
                        if (roleError) console.error('Global Setup: ERR promoting user in profiles:', roleError);
                        else console.log(`Global Setup: User ${userId} promoted to admin in profiles table.`);

                        // 2. Update Auth user_metadata (used by frontend for quick role checks)
                        const { data: updateData, error: authError } = await supabase.auth.admin.updateUserById(userId, {
                            user_metadata: { role: 'admin' }
                        });

                        if (authError) console.error('Global Setup: ERR updating user_metadata:', authError);
                        else {
                            console.log('Global Setup: user_metadata updated with role: admin');
                ...
                if (session) {
                    ...
                    await page.addInitScript((params) => {
                        window.localStorage.setItem(params.key, JSON.stringify(params.val));
                    }, { key: storageKey, val: session });

                    // Inject cookies BEFORE navigation so middleware sees them
                    const sessionStr = JSON.stringify(session);
                    console.log('Global Setup: Injecting auth cookies before navigation (raw JSON)...');
                    ...
                    try {
                        ...
                        console.log('Global Setup: Cookies found:', cookies.map(c => c.name));
                    } catch (e) { console.warn('Global Setup: Navigation/Sync timeout.', e); }

                } else {
                    // Fallback to navigating link if verification failed
                    console.log('Global Setup: Verification failed, trying direct link navigation...');
                    await page.goto(data.properties.action_link);
                    try { await page.waitForURL('**/dashboard', { timeout: 10000 }); } catch (e) { }
                }
            } else {
                console.warn('Global Setup: ⚠️ No action_link returned. User might not exist or config error.');
            }

        } else {
            console.warn('Global Setup: ⚠️ SUPABASE_SERVICE_ROLE_KEY not found. Falling back to manual form login (Subject to Rate Limits).');
            ...
            if (await page.getByTestId('auth-email-input').isVisible()) {
                ...
                await page.getByTestId('auth-password-input').fill(process.env.TEST_USER_PASSWORD || 'password');
