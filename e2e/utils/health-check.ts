...
export async function runUniversalHealthCheck(page: Page) {
	...
	for (let i = 0; i < imageCount; i++) {
		...
		if (isVisible) {
			...
			expect(isBroken, `Detected broken image: ${await images.nth(i).getAttribute('src')}`).toBe(false);
		}
	}
	...
	for (let i = 0; i < Math.min(linkCount, 15); i++) {
		...
		if (href && !href.startsWith('#') && !href.startsWith('mailto:') && !href.startsWith('tel:')) {
			const absoluteUrl = href.startsWith('http') ? href : `http://127.0.0.1:3000${href}`;

			linkChecks.push(
				page.request.get(absoluteUrl).then(response => {
					expect(response.status(), `Broken link detected: ${href}`).toBeLessThan(400);
				}).catch(err => {
	...
	const visibilityIssues = await page.evaluate(() => {
		...
		elements.forEach(el => {
			...
			if (color === bgColor && color !== 'rgba(0, 0, 0, 0)' && bgColor !== 'rgba(0, 0, 0, 0)') {
				issues.push(`Hidden text detected (same color as bg): ${el.tagName} "${el.textContent?.substring(0, 20)}"`);
			}

			// Check if element is effectively invisible but supposed to be text
			const rect = el.getBoundingClientRect();
			if (rect.width > 0 && rect.height > 0) {
				const isTransparent = style.color === 'rgba(0, 0, 0, 0)' || style.color === 'transparent';
				if (isTransparent && el.textContent?.trim().length) {
					issues.push(`Transparent text detected: ${el.tagName} "${el.textContent?.substring(0, 20)}"`);
				}
			}
		});
		return issues;
	...
	if (visibilityIssues.length > 0) {
		console.warn(`Potential Visibility Issues found on ${page.url()}:`, visibilityIssues);
	}

	expect(consoleErrors, `Console errors detected on ${page.url()}: ${consoleErrors.join(', ')}`).toHaveLength(0);
}
