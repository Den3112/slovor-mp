...
test.describe('Admin Moderation', () => {
    test('Admin can approve a pending listing', async ({ page }) => {
        ...
        await page.route('**', async (route) => {
            ...
            if (['image', 'stylesheet', 'font', 'script'].includes(type) || url.includes('.js') || url.includes('.css')) {
        ...
        // Verify page loaded
        await expect(page.getByRole('heading', { name: /moderation/i })).toBeVisible({ timeout: 15000 });
        ...
        const approveBtn = page.locator('button').filter({ has: page.locator('.lucide-check-circle-2') }).first();
        ...
        const draftBtn = page.locator('button').filter({ has: page.locator('.lucide-shield-alert') }).first();
