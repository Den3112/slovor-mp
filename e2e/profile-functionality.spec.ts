...
async function ensureAuthenticated(page) {
    ...
    if (currentUrl.includes('/auth/login')) {
        ...
        await page.waitForURL('**/profile/overview', { timeout: 15000 });
        await page.waitForLoadState('domcontentloaded');
...
test.describe('Mobile Profile View', () => {
    ...
    test('Mobile Navigation interactions', async ({ page }) => {
        ...
        await page.waitForURL('**/profile/listings', { timeout: 15000 });
        expect(page.url()).toContain('/profile/listings');

        // Click on Messages
        await page.getByTestId('profile-mobile-nav-messages').click();
        await page.waitForURL('**/profile/messages', { timeout: 15000 });
        expect(page.url()).toContain('/profile/messages');
