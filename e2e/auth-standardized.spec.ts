...
test.describe('Standardized Authentication Flow', () => {
	...
	test('Toggle between Login and Register works reliably', async () => {
		...
		await expect(authPage.page.locator('h1')).toContainText(/Join Slovor|Pridať sa|Pridat se|Присоединяйся/i);
		await authPage.expectNoForgotLink();

		// Toggle back
		await authPage.toggleToRegister();
		await expect(authPage.page.locator('h1')).toContainText(/Welcome Back|Vitajte späť|С возвращением/i);
	});
	...
	test('Empty inputs validation', async () => {
		...
		const isEmailValid = await authPage.emailInput.evaluate((el: HTMLInputElement) => el.validity.valid);
