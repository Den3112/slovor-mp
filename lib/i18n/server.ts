...
export async function getTranslationServer(ns: string | string[] = 'common') {
  ...
  const headerLocale = headerStore.get('x-slovor-locale')
  const cookieLocale = cookieStore.get(LOCALE_COOKIE_KEY)?.value

  const locale = (languages.includes(headerLocale as any) ? headerLocale :
    (languages.includes(cookieLocale as any) ? cookieLocale : fallbackLng)) as string
