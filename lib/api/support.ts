...
export const supportApi = {
    async createTicket(ticket: Omit<SupportTicket, 'id' | 'created_at' | 'updated_at' | 'status'>): Promise<ApiResponse<SupportTicket>> {
        try {
    ...
    async sendMessage(ticketId: string, message: string, isAdmin = false): Promise<ApiResponse<SupportMessage>> {
        try {
            const { data: { user } } = await supabase.auth.getUser()
            if (!user) throw new Error('Not authenticated')
    ...
    async updateStatus(id: string, status: SupportTicket['status']): Promise<ApiResponse<null>> {
        try {
