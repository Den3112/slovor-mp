...
import { LayoutDashboard, ShieldCheck, Users, AlertTriangle, Menu } from 'lucide-react'
...
export function AdminMobileNav({ onMenuClick }: AdminMobileNavProps) {
    ...
    return (
        <div className="fixed right-0 bottom-0 left-0 z-40 md:hidden">
            ...
            <nav className="relative flex h-16 items-center justify-around px-2 pb-safe">
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-widest truncate max-w-[60px]">{item.label}</span>
                        </Link>
                    )
                })}
                ...
                >
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest">{t('common:more')}</span>
                </button>
            </nav>
        </div>
    )
