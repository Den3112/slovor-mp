            ...
            cell: (row) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-sm font-bold tracking-tight text-foreground truncate max-w-[150px] leading-none mb-1">
                ...
                <div className="flex flex-col gap-1">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button
                                variant="ghost"
                                className="h-auto p-0 hover:bg-transparent justify-start font-bold uppercase text-xs tracking-wider text-primary"
                            >
                                {row.role || 'user'}
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="start">
                            ...
                            <DropdownMenuItem onClick={() => handleRoleChange(row, 'moderator')}>
                                Moderator
                            </DropdownMenuItem>
            ...
            cell: (row) => (
                <div className="flex flex-wrap gap-2">
                    {row.is_verified && (
                        <Badge
                            variant="outline"
                            className="bg-success/10 text-success border-success/20 font-bold text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-md"
                        >
                            Verified
                        </Badge>
                    )}
                    {row.status === 'banned' ? (
                        <Badge
                            variant="outline"
                            className="bg-destructive/10 text-destructive border-destructive/20 font-bold text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-md"
                        >
                            Banned
                        </Badge>
                    ) : (
                        <Badge
                            variant="outline"
                            className="bg-muted text-muted-foreground/40 border-border/40 font-bold text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-md"
                        >
                            Active
                        </Badge>
                    )}
