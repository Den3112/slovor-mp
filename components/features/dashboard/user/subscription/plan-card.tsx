  ...
  return (
    <div
      className={cn(
        ...
        plan.highlight ? "bg-primary/3 shadow-2xl shadow-primary/20" : "bg-white/2"
      )}
    >
      {/* Animated Background for Highlighted */}
      {plan.highlight && !isCurrent && (
        <div className="bg-primary/10 absolute -top-24 -right-24 -z-10 h-64 w-64 rounded-full blur-[100px] animate-pulse" />
      )}

      {/* Plan Header */}
      <div className="glass-panel relative overflow-hidden rounded-[2.5rem] border-none p-10">
        <div className="bg-blue-500/3 absolute inset-0 -z-10 blur-3xl" />
        ...
        <div className="flex items-center justify-between">
          ...
          {isCurrent ? (
            <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 rounded-xl px-3 py-1 text-[10px] font-black tracking-widest uppercase shadow-lg">
              {t('dashboard:subscriptions.active')}
            </Badge>
          ) : (
            plan.highlight && (
              <Badge className="bg-primary text-primary-foreground absolute -top-2 -right-2 rotate-12 rounded-xl px-3 py-1 text-[10px] font-black tracking-widest uppercase shadow-xl ring-4 ring-background/50">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground/60 text-xs font-bold tracking-tight leading-relaxed">
      ...
      <div className="border-border/5 mt-auto flex-1 border-t p-8 pt-0">
        <ul className="space-y-5 pt-8">
          {plan.features.map((feature, idx) => (
            ...
            >
              <div className="bg-emerald-500/20 text-emerald-400 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full shadow-lg shadow-emerald-500/10">
          ...
          className={cn(
            ...
            isCurrent
              ...
              : plan.highlight
                ...
                : 'bg-foreground text-background shadow-black/20 shadow-xl hover:bg-foreground/90 hover:scale-[1.05] active:scale-[0.98]'
          )}
