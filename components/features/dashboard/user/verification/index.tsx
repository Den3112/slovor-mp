...
export function VerificationView() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 mx-auto max-w-5xl space-y-12 p-4 md:p-8 duration-700">
        ...
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-center">
          <div className="space-y-3">
            <h1 className="flex items-center gap-4 text-4xl font-black tracking-tighter uppercase sm:text-5xl">
              <div className="bg-primary/10 flex h-16 w-16 items-center justify-center rounded-2xl border border-primary/20 shadow-xl shadow-primary/10">
      ...
      <div className="grid gap-6">
        {steps.map((step, index) => (
          ...
          >
            <div
              className={cn(
                'glass-panel group relative border-none p-8 shadow-xl shadow-black/5 transition-all duration-500 hover:bg-primary/2',
              ...
              <div className="flex flex-col gap-8 md:flex-row md:items-center">
                  ...
                  className={cn(
                    ...
                    step.isVerified
                      ...
                      : step.isRejected
                        ...
                        : 'border-white/10 bg-white/5 text-white shadow-black/20 group-hover:border-primary/30 group-hover:bg-primary/5 group-hover:text-primary group-hover:shadow-primary/20'
                  )}
                >
                  <step.icon className="h-10 w-10" />
                  {step.isVerified && (
                    <div className="bg-emerald-500 absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full shadow-lg">
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-4">
                    <h3 className="text-foreground text-3xl font-black tracking-tighter uppercase transition-colors group-hover:text-primary">
                      {step.title}
                    </h3>
                    {step.isVerified && (
                      <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 rounded-xl px-2.5 py-1 text-[9px] font-black tracking-widest uppercase shadow-sm">
                        {t('verification:verified')}
                      </Badge>
                    )}
                    {step.isPending && (
                      <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30 animate-pulse rounded-xl px-2.5 py-1 text-[9px] font-black tracking-widest uppercase shadow-sm">
                        {t('verification:pending')}
                      </Badge>
                    )}
                    {step.isRejected && (
                  ...
                  <p className="text-muted-foreground/60 max-w-2xl text-[13px] font-bold tracking-tight leading-relaxed">
                    {step.description}
                  </p>
                </div>
                ...
                <div className="shrink-0">
                  {step.isVerified ? (
                    <div className="bg-emerald-500/10 text-emerald-400 flex items-center gap-3 rounded-2xl px-6 py-3 shadow-lg shadow-emerald-500/5 transition-transform hover:scale-105">
                    ...
                    <div className="bg-amber-500/10 text-amber-400 flex items-center gap-3 rounded-2xl px-6 py-3 shadow-lg shadow-amber-500/5">
      ...
      {/* Info Card - High End */}
      <div className="relative overflow-hidden rounded-[2.5rem] p-10">
        <div className="bg-blue-500/5 absolute inset-0 -z-10 backdrop-blur-3xl" />
        <div className="flex gap-8">
          <div className="bg-blue-500/10 flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-blue-500/20 shadow-2xl shadow-blue-500/10">
            <AlertCircle className="text-blue-400 h-9 w-9" />
          </div>
          <div className="space-y-3">
            <h4 className="text-2xl font-black tracking-tighter uppercase sm:text-3xl">
              {t('verification:whyVerify')}
            </h4>
            <p className="text-muted-foreground/60 text-sm font-bold tracking-tight leading-relaxed">
            ...
            <div className="space-y-10">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="bg-white/5 h-12 w-12 rounded-xl transition-transform hover:rotate-90"
                >
                  <Plus className="stroke-3 h-4 w-4" />
                </Button>
              </div>
                    ...
                    <Button
                      ...
                      className={cn(
                        ...
                        selectedDocType === type
                          ? 'border-primary bg-primary/10 text-primary shadow-2xl! shadow-primary/20 scale-105'
                          : 'glass-panel border-white/5 bg-white/2 hover:bg-primary/2 p-6 transition-all duration-300 text-muted-foreground/40 hover:border-primary/30 hover:text-primary'
                      )}
                    >
              ...
              <div className="bg-white/2 group border-white/5 hover:border-primary/40 relative flex flex-col items-center justify-center rounded-[2.5rem] border-2 border-dashed py-16 transition-all duration-500 hover:bg-primary/2">
                <div className="bg-background/50 border-white/10 flex h-20 w-20 items-center justify-center rounded-2xl border shadow-2xl transition-transform duration-700 group-hover:scale-110 group-hover:rotate-12">
                  <Loader2
                    className={cn(
                ...
                <div className="mt-8 space-y-2 text-center">
                  <p className="text-lg font-black tracking-tighter uppercase transition-colors group-hover:text-primary">
              ...
              <div className="flex gap-4">
                <Button
                  variant="outline"
                  className="h-14 flex-1 rounded-2xl text-[11px] font-black tracking-[0.2em] uppercase border-white/5 bg-white/5 hover:bg-white/10"
                  onClick={() => setShowingUploadModal(false)}
                  disabled={isSubmitting}
                >
                  {t('common:cancel')}
