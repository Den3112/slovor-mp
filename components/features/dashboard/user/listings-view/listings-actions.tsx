...
export function BulkActionsBar({
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/10 h-10 w-px" />
      <div className="flex items-center gap-3">
        <Button
          size="sm"
          variant="ghost"
          className="hover:bg-white/10 h-10 rounded-2xl px-5 text-[9px] font-black tracking-[0.15em] text-white uppercase transition-all active:scale-95"
          onClick={onCancel}
        >
        ...
        <Button
          size="sm"
          className="hover:bg-white/90 h-10 rounded-2xl bg-white px-6 text-[9px] font-black tracking-[0.15em] text-slate-900 uppercase transition-all active:scale-95 shadow-lg shadow-white/10"
...
export function BulkConfirmDialog({
  ...
  return (
    <AlertDialog open={!!action} onOpenChange={(open) => !open && onClose()}>
      <AlertDialogContent className="glass-panel border-primary/10 bg-background/80 rounded-[2.5rem] shadow-2xl backdrop-blur-3xl overflow-hidden p-0 max-w-md">
        <div className="border-primary/5 bg-primary/5 p-8 border-b">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-foreground text-2xl font-black tracking-tighter uppercase mb-2">
              {action === 'delete'
                ? t('dashboard:bulkActions.deleteTitle')
                : t('dashboard:bulkActions.deactivateTitle')}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-foreground/60 text-sm font-black tracking-tight leading-relaxed">
              {action === 'delete'
        ...
        <div className="p-8">
          <AlertDialogFooter className="flex-row items-center gap-3 sm:justify-end">
            ...
            <AlertDialogAction
              ...
              className={cn(
                'h-12 flex-1 rounded-2xl text-[10px] font-black tracking-[0.2em] uppercase transition-all active:scale-95 shadow-xl sm:flex-none sm:px-8',
                action === 'delete'
