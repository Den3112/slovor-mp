...
import { FlagSK, FlagUS, FlagCZ, SUPPORTED_LOCALES } from '@/components/icons/flags'
    ...
    return (
        <div className="relative">
            <button
                ...
                className="flex items-center gap-2 rounded-full border border-border/40 bg-muted/20 px-3 py-2.5 text-[10px] font-black uppercase tracking-widest text-foreground transition-all hover:bg-muted/40"
            >
                <div className="flex h-4 w-5 items-center justify-center overflow-hidden rounded-sm bg-muted shadow-sm">
            ...
            <AnimatePresence>
                {showLangMenu && (
                    <motion.div
                        ...
                        className="shadow-premium absolute right-0 z-50 mt-3 w-44 overflow-hidden rounded-2xl border border-border bg-card/90 p-2 backdrop-blur-2xl"
                    >
                        {SUPPORTED_LOCALES.map((lang) => (
                            ...
                            >
                                <div className="flex h-4 w-6 shrink-0 items-center justify-center overflow-hidden rounded-sm bg-muted">
                                    <div className="h-full w-full scale-125 transform saturate-[1.2]">
                                        {lang.flag}
                                    </div>
                                </div>
...
export function MobileLanguageSelector({ locale, setLocale }: MobileLanguageSelectorProps) {
    return (
        <div className="mt-8">
