...
export function UserMenu({ user, signOut }: UserMenuProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        <div className="from-primary shadow-primary/10 h-9 w-9 rounded-lg bg-linear-to-tr via-violet-500 to-indigo-500 p-[1.5px] shadow-lg transition-transform group-hover:scale-105">
          <div className="border-primary/10 bg-card text-primary relative flex h-full w-full items-center justify-center overflow-hidden rounded-lg border font-bold">
      ...
      <AnimatePresence>
        {showUserMenu && (
          <motion.div
            ...
            className="shadow-premium border-border bg-card/95 absolute right-0 z-50 mt-3 w-60 overflow-hidden rounded-lg border"
          >
            ...
            <div className="space-y-0.5 p-2">
              {config.app.adminEmails.includes(user.email || '') && (
                <Link
                  href={`/${locale}/admin`}
                  className="group flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-bold text-amber-500 transition-all hover:bg-amber-500/10 hover:text-amber-600"
              ...
              <button
                onClick={() => signOut()}
                className="text-destructive hover:bg-destructive/5 flex w-full items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-bold transition-all"
              >
