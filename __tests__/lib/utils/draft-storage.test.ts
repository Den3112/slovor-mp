...
import { saveListingDraft, loadListingDraft, clearListingDraft } from '@/lib/utils/draft-storage'
...
describe('draft-storage utility', () => {
    ...
    it('returns null if draft is expired', () => {
        ...
        const key = `listing-draft:${mockUser}`
        localStorage.setItem(key, JSON.stringify({ data: mockData, timestamp: oldTimestamp }))
    ...
    it('handles storage errors', () => {
        ...
        const setItemSpy = vi.spyOn(localStorage, 'setItem').mockImplementation(() => { throw new Error('Full') })
