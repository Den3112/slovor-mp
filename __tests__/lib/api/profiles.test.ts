...
describe('profilesApi', () => {
    ...
    describe('update', () => {
        it('upserts profile', async () => {
            ...
            const response = await profilesApi.update('1', { display_name: 'Updated' })
            expect(response.error).toBeNull()
            expect(response.data).toEqual(mockProfile)
        ...
        it('handles failure in update', async () => {
            const maybeSingleMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'Update Failed' } })
            const selectMock = vi.fn().mockReturnValue({ maybeSingle: maybeSingleMock })
            const upsertMock = vi.fn().mockReturnValue({ select: selectMock })
    ...
    describe('getStats', () => {
        it('calculates profile statistics correctly', async () => {
            ...
            const favoritesEqMock = vi.fn().mockResolvedValue({ count: 15, error: null })
        ...
        it('handles errors when fetching listings', async () => {
            const listingsEqMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'DB Error' } })
            const listingsSelectMock = vi.fn().mockReturnValue({ eq: listingsEqMock })
    ...
    describe('getRecentActivity', () => {
        it('fetches recent listings', async () => {
            ...
            const limitMock = vi.fn().mockResolvedValue({ data: mockListings, error: null })
        ...
        it('handles errors', async () => {
            const limitMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'Fetch Fail' } })
            const orderMock = vi.fn().mockReturnValue({ limit: limitMock })
            const eqMock = vi.fn().mockReturnValue({ order: orderMock })
            const selectMock = vi.fn().mockReturnValue({ eq: eqMock })
