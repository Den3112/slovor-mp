...
describe('savedSearchesApi', () => {
    ...
    describe('create', () => {
        it('creates saved search', async () => {
            ...
            expect(response.data).toEqual(mockData)
            expect(insertMock).toHaveBeenCalledWith(expect.objectContaining({ user_id: 'u1', name: 'New Search' }))
        })
    })
    ...
    describe('update', () => {
        it('updates saved search', async () => {
            ...
            const singleMock = vi.fn().mockResolvedValue({ data: { id: '1', ...updates }, error: null })
            const selectMock = vi.fn().mockReturnValue({ single: singleMock })
    ...
    describe('saveCurrentSearch', () => {
        it('calls create with formatted input', async () => {
            const createSpy = vi.spyOn(savedSearchesApi, 'create').mockResolvedValue({ data: {}, error: null } as any)

            await savedSearchesApi.saveCurrentSearch('Name', { query: 'q', minPrice: 10 })
