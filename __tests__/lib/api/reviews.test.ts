...
describe('reviewsApi', () => {
    ...
    describe('create', () => {
        it('creates review successfully', async () => {
            const input = { seller_id: 's1', buyer_id: 'b1', rating: 5, comment: 'Great' }
            const mockReview = { id: '1', ...input }

            const singleMock = vi.fn().mockResolvedValue({ data: mockReview, error: null })
        ...
        it('prevents self-review', async () => {
            const response = await reviewsApi.create({ seller_id: 'u1', buyer_id: 'u1', rating: 5 })
            expect(response.error).toContain('cannot review yourself')
        })

        it('validates rating range', async () => {
            const response = await reviewsApi.create({ seller_id: 's1', buyer_id: 'b1', rating: 6 })
    ...
    describe('hasReviewed', () => {
        it('checks if reviewed (with listing)', async () => {
            ...
            const eqMock = vi.fn().mockReturnValue({ maybeSingle: maybeSingleMock, eq: vi.fn(), select: vi.fn() })
