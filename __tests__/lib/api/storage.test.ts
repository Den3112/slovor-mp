...
describe('storageApi', () => {
    ...
    describe('uploadImage', () => {
        ...
        it('uploads valid image successfully', async () => {
            ...
            const uploadMock = vi.fn().mockResolvedValue({ data: { path: 'path/to/img.jpg' }, error: null })
            const getUrlMock = vi.fn().mockReturnValue({ data: { publicUrl: 'http://url/img.jpg' } })
        ...
        it('handles upload error', async () => {
            ...
            const uploadMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'Upload Failed' } })
    ...
    describe('uploadImages', () => {
        it('uploads multiple images', async () => {
            ...
            const uploadSpy = vi.spyOn(storageApi, 'uploadImage')
                .mockResolvedValueOnce({ data: { url: 'url1' } as any, error: null })
                .mockResolvedValueOnce({ data: { url: 'url2' } as any, error: null })

            const progressSpy = vi.fn()

            const response = await storageApi.uploadImages(files, 'user-1', progressSpy)
        ...
        it('handles failure when no files uploaded', async () => {
            ...
            vi.spyOn(storageApi, 'uploadImage')
                .mockResolvedValue({ data: null, error: 'Fail' })
    ...
    describe('deleteImage', () => {
        ...
        it('handles delete error', async () => {
            const removeMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'Remove Failed' } })
            vi.mocked(supabase.storage.from).mockReturnValue({ remove: removeMock } as any)
