...
describe('middleware utility', () => {
    ...
    const mockRequest = (pathname: string) => ({
        nextUrl: {
            ...
            clone: function (this: any) {
                const self = this;
                return {
                    get pathname() { return self.pathname },
                    set pathname(val) { self.pathname = val },
                    toString() { return self.pathname }
                }
            },
        },
        cookies: {
    ...
    it('handles cookie setting and removal inside createServerClient options', async () => {
        ...
        expect(req.cookies.set).toHaveBeenCalledWith(expect.objectContaining({ value: '' }))
    })
})
