...
import { getAuthenticatedClient, createErrorResponse, createSuccessResponse } from '@/app/api/utils'
...
describe('API Utils', () => {
    describe('getAuthenticatedClient', () => {
        ...
        it('returns null if header is empty/invalid', () => {
            const req = new NextRequest('http://localhost', { headers: { Authorization: 'Bearer ' } })
            const client = getAuthenticatedClient(req)
            expect(client).toBeNull()
        })
        ...
        it('creates client with valid token', () => {
            const req = new NextRequest('http://localhost', { headers: { Authorization: 'Bearer valid-token' } })
            const client = getAuthenticatedClient(req)
