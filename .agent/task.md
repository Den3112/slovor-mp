...
- [x] **Drag-and-Drop Uploader (Sleek UI)**
    - [x] Реализовать красивую зону загрузки с использованием `framer-motion`.
    - [x] Добавить визуальные подсказки при наведении файла (Hover states).
- [x] **Live Previews & Micro-interactions**
    - [x] Анимированные превью изображений (появление, удаление).
    - [x] Индикаторы прогресса загрузки для каждого файла отдельно (индикация текущего файла).
- [x] **Image Optimization & Management**
    - [x] Проверить/улучшить логику удаления из Supabase Storage при нажатии "X".
    - [x] Реализовать сортировку (Drag-to-reorder) для выбора главного фото (Cover).
- [x] **Responsive Media Performance**
    - [x] Оптимизация загрузки через Next.js Image и blur-up плейсхолдеры. (Внедрено в ListingCard и ImageGallery).
