...
async function seedChats() {
    ...
    // 1. Get all users
    const { data: users, error: usersError } = await supabase.from('profiles').select('id, display_name')
    ...
    // 2. Get some listings
    const { data: listings, error: listingsError } = await supabase.from('listings').select('id, title, user_id').limit(10)
    ...
    for (const user of users) {
        ...
        if (!targetListing) {
            console.log(`Skipping user ${user.display_name} (only owns listings or no others available)`)
            continue
        }

        console.log(`Creating chat for ${user.display_name} regarding "${targetListing.title}"`)
